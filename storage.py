...
Claim, heartbeat, and terminal submission each lock the parent task row with
``SELECT ... FOR UPDATE`` (``SKIP LOCKED`` for claims) to make concurrent
access safe; see the note on :func:`database.recover_expired_in_session` for
the locking convention they all share.
"""
...
def _idempotent_match(existing: Task, recipient_id: str, input_text: str) -> dict[str, str]:
    ...
    with db_session() as db:
        ...
        if idempotency_key is not None:
            ...
            if existing is not None:
                return _idempotent_match(existing, recipient_id, input_text)
        task = Task(
            id=new_id("task"),
            sender_id=sender_id,
        ...
        try:
            ...
            # Two concurrent requests with the same sender-scoped Idempotency-Key
            # can both pass the check above; the unique constraint catches the
            # race, and we fall back to the same idempotent-match response.
            db.rollback()
...
def claim_one(agent_id: str, worker_id: str | None) -> dict[str, Any] | None:
    with db_session() as db:
        ...
        # SKIP LOCKED lets a concurrent claim for the same agent move past a
        # task another worker is already claiming, instead of blocking on it.
        task = db.scalar(
            select(Task)
